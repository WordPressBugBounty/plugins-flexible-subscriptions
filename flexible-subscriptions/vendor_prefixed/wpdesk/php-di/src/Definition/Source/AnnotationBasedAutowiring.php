...
use WPDesk\FlexibleSubscriptions\Vendor\DI\Annotation\Inject;
use WPDesk\FlexibleSubscriptions\Vendor\DI\Annotation\Injectable;
use WPDesk\FlexibleSubscriptions\Vendor\DI\Definition\Exception\InvalidAnnotation;
use WPDesk\FlexibleSubscriptions\Vendor\DI\Definition\ObjectDefinition;
use WPDesk\FlexibleSubscriptions\Vendor\DI\Definition\ObjectDefinition\MethodInjection;
use WPDesk\FlexibleSubscriptions\Vendor\DI\Definition\ObjectDefinition\PropertyInjection;
use WPDesk\FlexibleSubscriptions\Vendor\DI\Definition\Reference;
use WPDesk\FlexibleSubscriptions\Vendor\Doctrine\Common\Annotations\AnnotationRegistry;
use WPDesk\FlexibleSubscriptions\Vendor\Doctrine\Common\Annotations\Reader;
use WPDesk\FlexibleSubscriptions\Vendor\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use WPDesk\FlexibleSubscriptions\Vendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'WPDesk\FlexibleSubscriptions\Vendor\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'WPDesk\FlexibleSubscriptions\Vendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('WPDesk\FlexibleSubscriptions\Vendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'WPDesk\FlexibleSubscriptions\Vendor\DI\Annotation\Injectable');
