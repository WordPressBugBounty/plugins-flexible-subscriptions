...
use WPDesk\FlexibleSubscriptions\Vendor\WPDesk\Interval\StringSpec;
use WPDesk\FlexibleSubscriptions\Vendor\WPDesk\Interval\WPInterval;

/**
 * As the rule of the thumb, most of getters and setters are only for
 * internal use, to fill the props of the class. Interaction with
 * Subscription object should be done by dedicated object values
...
class Subscription extends \WC_Order {
	...
	private int $lifecycle_transition_depth = 0;
	...
	public function __toString(): string {
		return json_encode(
			[
				...
				'recent_payment_request_id'   => $this->get_recent_payment_request_id(),
				'payment_method'              => $this->get_payment_method(),
				'billing_frequency'           => (string) $this->get_billing_frequency(),
				'current_period_start_date'   => $this->get_current_period_start() ? $this->get_current_period_start()->format( 'c' ) : null,
				'current_period_end_date'     => $this->get_current_period_end() ? $this->get_current_period_end()->format( 'c' ) : null,
				'trial_end_date'              => $this->get_trial_end_date() ? $this->get_trial_end_date()->format( 'c' ) : null,
				'end_date'                    => $this->get_end_date() ? $this->get_end_date()->format( 'c' ) : null,
			]
		);
	}
	 ...
	 *
	 * @deprecated 1.7.0 Use SubscriptionLifecycleManager::can_transition().
	 */
	...
	public function payment_complete( $transaction_id = '' ) {
		...
		if ( ! $this->is_active() ) {
			$request = new LifecycleTransitionRequest( $this, 'active', TransitionContext::system( 'payment_complete' ) );
			do_action( 'fsub/_internal/subscription/lifecycle/transition', $request );
			if ( ! $request->handled ) {
	...
	/** @internal */
	public function begin_lifecycle_transition(): void {
		++$this->lifecycle_transition_depth;
	}

	/** @internal */
	public function end_lifecycle_transition(): void {
		$this->lifecycle_transition_depth = max( 0, $this->lifecycle_transition_depth - 1 );
	}

	/** @internal */
	public function is_lifecycle_transition_in_progress(): bool {
		return $this->lifecycle_transition_depth > 0;
	}

	/**
	 * @deprecated 1.7.0 Use SubscriptionLifecycleManager::transition( $subscription, 'active', ... ).
	 */
	public function activate(): void {
		$request = new LifecycleTransitionRequest( $this, 'active', TransitionContext::system( 'activate' ) );
		do_action( 'fsub/_internal/subscription/lifecycle/transition', $request );
		if ( ! $request->handled ) {
			$this->update_status( 'active' );
		}
	...
	/**
	 * @deprecated 1.7.0 Use SubscriptionLifecycleManager::transition( $subscription, 'pending-cancel', ... ).
	 */
	public function cancel(): void {
		if ( $this->has_status( 'pending' ) && $this->is_during_first_cycle() ) {
			$request = new LifecycleTransitionRequest( $this, 'cancelled', TransitionContext::manual( 'cancel' ) );
			do_action( 'fsub/_internal/subscription/lifecycle/transition', $request );
			if ( ! $request->handled ) {
		...
		$request = new LifecycleTransitionRequest( $this, 'pending-cancel', TransitionContext::manual( 'cancel' ) );
		do_action( 'fsub/_internal/subscription/lifecycle/transition', $request );
		if ( ! $request->handled ) {
			$this->update_status( 'pending-cancel' );
		}
	}
	...
	public function has_status( $status ): bool {
		...
		if ( $check_with_prefix === false ) {
			$status = array_map(
				fn ( $s ) => str_starts_with( $s, 'wc-' ) ? substr( $s, 3 ) : $s,
				(array) $status
			);
	...
	public function update_status( $new_status, $note = '', $manual = false ): bool {
		...
		if ( str_starts_with( $new_status, 'wc-' ) ) {
	...
	/**
	 * Trigger internal status change if status actually changed.
	 *
	 * @internal
	...
	/**
	 * TODO: introduce strategy which will be able to snap periods to
	 * specific intervals?
	 */
	public function recalculate_periods(): void {
		$start = $this->get_current_period_start() ?? $this->get_start_date();
		...
		$this->set_current_period_start( $start );

		if ( $this->get_start_date() === $start && $this->has_trial() ) {
			// Trial can be modified only before any payments are made.
			if ( $this->get_trial_end_date() instanceof \DateTimeInterface ) {
				$this->set_current_period_end( $this->get_trial_end_date() );
			} elseif ( $this->get_trial_interval() instanceof WPInterval ) {
				// Backward compatibility with trial interval.
				$this->set_current_period_end( $start->add( $this->get_trial_interval() ) );
			}
		} else {
			$this->set_current_period_end(
				$start->add( $this->get_billing_frequency() )
			);
		}

		if ( $this->get_expiration() instanceof WPInterval ) {
			$end_date = $start->add( $this->get_expiration() );
			if ( $this->has_trial() ) {
				if ( $this->get_trial_end_date() instanceof \DateTimeInterface ) {
					$end_date = $end_date->add( $this->get_trial_end_date()->diff( $start, true ) );
				} elseif ( $this->get_trial_interval() instanceof WPInterval ) {
					// Backward compatibility with trial interval.
					$end_date = $end_date->add( $this->get_trial_interval() );
				}
			}
	 ...
	 */
	public function advance_billing_period(): bool {
		$current_end = $this->get_current_period_end();
		if ( ! $current_end instanceof \DateTimeInterface ) {
			return false;
		}

		if ( $current_end instanceof \DateTimeImmutable ) {
			$current_end_utc = $current_end;
		} elseif ( $current_end instanceof \DateTime ) {
			$current_end_utc = \DateTimeImmutable::createFromMutable( $current_end );
		} else {
			$current_end_utc = new \DateTimeImmutable( $current_end->format( 'Y-m-d H:i:s' ), $current_end->getTimezone() );
		}

		$current_end_utc = $current_end_utc->setTimezone( new \DateTimeZone( 'UTC' ) );
		$new_start       = $current_end_utc;
		$new_end         = $new_start->add( $this->get_billing_frequency() );

		$this->set_current_period_start( $new_start );
		$this->set_current_period_end( $new_end );
